import type { SummonerProfileResponse } from '../models'

import { useFetch } from '@vueuse/core'
type RiotName = {
    gameName: string
    tagLine: string
}
type Response<T> = { status: 'success'; data: T } | { status: 'error'; message: string }

export interface Repository<SummonerProfileResponse> {
    getLeagueProfile: (riotName: string) => Promise<Response<SummonerProfileResponse>>
    getRiotName: (riotName: string) => RiotName
}

export class BlitzRepository implements Repository<SummonerProfileResponse> {
    private readonly baseUrl: string

    constructor() {
        this.baseUrl =
            'https://riot.iesdev.com/graphql?query=query+LeagueProfile%28%24summoner_name%3AString+%24summoner_id%3AString+%24account_id%3AString+%24region%3ARegion%21+%24puuid%3AString+%24gameName%3AString+%24tagLine%3AString%29%7BleagueProfile%28summoner_name%3A%24summoner_name+summoner_id%3A%24summoner_id+account_id%3A%24account_id+region%3A%24region+puuid%3A%24puuid+gameName%3A%24gameName+tagLine%3A%24tagLine%29%7Branks%28first%3A30%29%7Bqueue+tier+rank+wins+losses+leaguePoints+insertedAt%7Did+accountId+puuid+summonerId+summonerName+summonerLevel+profileIconId+riotAccount%7Bid+puuid+gameName+tagLine%7DupdatedAt+latestRanks%7Bqueue+tier+rank+wins+losses+leaguePoints+insertedAt%7D%7D%7D&variables=%7B%22region%22%3A%22NA1%22%2C%22'
    }

    async getLeagueProfile(riotName: string): Promise<Response<SummonerProfileResponse>> {
        const riot_name = this.getRiotName(riotName)
        const url = `${this.baseUrl}gameName%22%3A%22${encodeURIComponent(riot_name.gameName)}%22%2C%22tagLine%22%3A%22${encodeURIComponent(riot_name.tagLine)}%22%7D`

        try {
            const { data: summonerData, statusCode } =
                await useFetch<SummonerProfileResponse>(url).json()

            if (statusCode.value === 200) {
                return { status: 'success', data: summonerData.value }
            } else {
                return { status: 'error', message: 'Failed to fetch data successfully' }
            }
        } catch (error) {
            return { status: 'error', message: `Error fetching data: ${error}` }
        }
    }

    getRiotName(riotName: string): RiotName {
        if (!riotName.includes('#')) {
            throw new Error('riotName must contain #')
        }
        const [gameName, tagLine] = riotName.split('#')
        return { gameName, tagLine }
    }
}
